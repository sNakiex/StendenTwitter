...
class MySQL
{
    ...
    public function Kill($message = "") {
        ...
        GLOBAL $config;
        $error = $this->Error();
        //$cms->reportError($error,"MYSQL","File ".$_SERVER["SCRIPT_NAME"]);
        $adm = "";
        //if($cms->hasAccess($cms->session("user_id"),"admin")){
        $adm = "<br><br>ADMIN DEBUG: ".$error;
        //}
        if($config->reportSiteErrors){
            echo "Something went wrong, it has been reported!".$adm;
        }else{
            echo "Something went wrong, if you would like to continue please contact a staff member".$adm;
        }
    }
    ...
    public function Release() {
        ...
        if (!$this->last_result) {
            ...
            if(!is_bool($this->last_result)) $success = mysql_free_result($this->last_result);
        ...
        switch (strtolower(trim($datatype))) {
            ...
            case "char":
                if (strlen($value) == 0) {
                    ...
                    $return_value  = "'".$cms->safe_string_escape($value)."'";
                }
                break;
            case "number":
            case "integer":
